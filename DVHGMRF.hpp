...
namespace HGMRF {
	...
	public:
		...
		mat denoising(const mat3D& noise)
		{
			...
			for (_epoch = 0; _epoch < _mepoch; ++_epoch) {
				Type error = 0.0;
				gauss_seidel_method(_noise3D, u, v, w, error);
#ifdef DEBUG_MODE 
	...
	private:
		...
		// ================================================================================
		// MAP estimation
		void gauss_seidel_method(const mat3D& noise, mat& u, mat& v, mat& w, Type& err) noexcept
		{
			...
			for (u32 iter = 0; iter < GSITER; ++iter) {
				...
				for (i32 i = 0; i < _cols; ++i) {
					for (i32 j = 0; j < _rows; ++j) {
						...
						Type u_numerator = _gammma2 * v[i][j];
						for (u32 k = 0; k < _enumerate; ++k) u_numerator += noise[k][i][j] / _vec_sigma2[k];
		...
		void pred_parameters(const mat3D& noise, const mat& u, const mat& v, const mat& w) noexcept
		{
			...
			for (u32 i = 0; i < _cols; ++i) {
				for (u32 j = 0; j < _rows; ++j) {
					...
					lambda_grad += 0.5 * _gammma2 * _gammma2 * w[i][j] * w[i][j] - u[i][j] * u[i][j] + 0.5 * (2 / first - 1 / second) * inv_sigma2 * 1 / chi;
					alpha_grad += 0.5 * (2 / first - 1 / second) * _eigen[i][j] * inv_sigma2 / chi;
					gammma2_grad += 0.5 * v[i][j] * v[i][j] - 0.5 * inv_sigma2 / (chi * second);
					for (u32 k = 0; k < _enumerate; ++k) sigma2_strict[k] += (noise[k][i][j] - u[i][j]) * (noise[k][i][j] - u[i][j]) + 1 / chi;
				}
			}
		...
		mat3D centerling3D(mat3D imgs) noexcept {
			...
			for (auto& img : imgs) for (auto& vec : img) for (auto& pix : vec) pix -= _expect;
			return imgs;
		}
		...
		mat centerling(mat img) noexcept {
			const Type _expect = this->expect;
			for (auto& vec : img) for (auto& pix : vec) pix -= _expect;
			return img;
		}
		...
		Type calc_expect(const mat3D& imgs) noexcept {
			Type _expect = 0.0;
			for (const auto& img : imgs) for (const auto& vec : img) for (const auto& pix : vec) _expect += pix;
		...
		mat averaging(const mat3D& imgs) noexcept {
			...
			for (const auto& img : imgs) {
				for (u32 i = 0; i < _cols; ++i) {
					for (u32 j = 0; j < _rows; ++j) {
						_avg_img[i][j] += img[i][j];
					}
				}
			}
			for (auto& vec : _avg_img) for (auto& pix : vec) pix /= _enumerate;
			return _avg_img;
		}
